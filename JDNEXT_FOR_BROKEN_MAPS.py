...
def fixing():
    ...
    for files in os.listdir("temp/"):
        ...
        if dtape['measureCount'] <= 60:
            print('[INFO] Fast move detected, using method...')
        else:
            print('[INFO] Slow move detected, using method...')
        
        for measure in dtape['measures']:
            if dtape['measureCount'] >= 60:
                randDivide = random.SystemRandom().uniform(1,1.5) #dont put more than or equal to 2 otherwise slow moves broken
                divide=measure/randDivide
                newmeasure=divide
                
            else:
                randDivide = random.SystemRandom().uniform(1.01,1.95) #ubi weird tbh
                divide=measure/randDivide
                newmeasure=divide
        ...
        dtape['moveAccurateLowThreshold']=random.SystemRandom().uniform(1, 1.2476837158)
        dtape['moveAccurateHighThreshold']=random.SystemRandom().uniform(4.5, 5.001)
        # since v5 ignores some variables...
        if dtape['version']==7:
            dtape['autoCorrelationThreshold']=1.2
            dtape['moveDirectionImpactFactor']=0.20000000298023224
