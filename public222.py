...
def fixing():
    ...
    for files in os.listdir("temp/"):
        ...
        if dtape['version'] == 7 or dtape['version'] == 5:
            for measure in dtape['measures']:
                if dtape['measureCount'] <= 60:
                    randDivide = random.SystemRandom().uniform(1.15,1.5) #ubi weird tbh
                    divide=measure/randDivide
                    newmeasure=divide
                    
                else:
                    randDivide = 1.05 #ubi weird tbh
                    divide=measure/randDivide
                    newmeasure=divide
        ...
        dtape['moveAccurateLowThreshold']=random.SystemRandom().uniform(1.2, 1.2000000476837158)
