...
def fixing():
    ...
    for files in os.listdir("temp/"):
        ...
        for measure in dtape['measures']:
            if dtape['measureCount'] <= 60:
                randDivide = random.SystemRandom().uniform(1.01,1.15) #dont put more than or equal to 2 otherwise slow moves broken
        ...
        dtape['moveAccurateLowThreshold']=random.SystemRandom().uniform(1.100000023841858,1.200000023841858)
        if dtape['moveDuration'] <= 0.8: # this is because we need to do less for a shorter move (coach moves their arm "small" compared to longer movements)
    ...
    for msm in os.listdir('temp'):
        if msm.endswith('.json'):
            msmname=msm.split('.')[0]
            Serialize('temp/'+msm, 'output/'+msmname+'.msm')
